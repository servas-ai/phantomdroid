...
class FullProbeRunnerSpoofTest {
    ...
    private fun allProbes(): List<Probe> = listOf(
        ...
        NetworkTypeProbe(),
        VpnProxyProbe(),
        // root (3) — Power-13 Gap #3 added MountNsMismatchProbe
    ...
    fun `full probe runner classifies spoofed snapshot as CLEAN with zero critical failures`() =
        runBlocking {
            ...
            assertEquals(
                75, probes.size,
                "expected the full 75-probe inventory (73 base + Power-13 Gap #4 " +
                    "ThirdPartyEmulatorArtifactsProbe + Power-13 Gap #3 " +
                    "MountNsMismatchProbe); if the inventory changed, update " +

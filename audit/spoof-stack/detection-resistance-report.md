...
### 5b.1 Test exercises probes via no-arg constructors only (with 1 exception)

**Finding**: The test instantiates every probe via its no-arg / default-arg constructor (except `BluetoothMacProbe` which wires `bluetoothAdapterMacSupplier = { ctx.queryBluetoothAdapterMac() }`). For all 10+ bucket-(c) probes — `GpuRendererProbe`, `MediaDrmProbe`, `AccountsProbe`, `NetworkTypeProbe`, `WifiSsidBssidProbe`, `DnsServerProbe`, `GaidProbe`, `GsfIdProbe`, `ScreenResolutionProbe`, `VpnProxyProbe.transportVpnFlagSupplier` — the constructor-injected suppliers default to `{ null }` and the probes degrade to score=0.0 / confidence=0.50 in the "no supplier returned" branches.

**Implication**: Some of the "0.0" scores in the residual list are not "the probe found a clean value" but "the probe couldn't see any value and gave up". When future iterations wire ProbeContext default-methods + SnapshotReplayContext overrides for these probes, the test must be updated to wire each probe through its ctx-delegating supplier (mirror the BluetoothMacProbe pattern at line ~150). Until that happens, the test under-tests bucket-(c) probes by silencing their primary signal.

> *Builder response*: Confirmed and partially addressed in Phase 4. The phase-4 refactor pattern (nullable supplier `(() -> X?)? = null` + ctx fallback when supplier is null) was applied to the residual-firing rank-23 `ScreenResolutionProbe` AND to the phase-3 rank-20 `TimezoneLocaleProbe` / rank-36 `LanguageCountryProbe`. The remaining bucket-(c) probes (GpuRenderer, MediaDrm, Accounts, NetworkType, WifiSsidBssid, DnsServer, Gaid, GsfId, VpnProxy.transportVpnFlagSupplier) **still use `() -> X? = { null }` defaults** because they did NOT fire in the phase-1 baseline residual list and applying the refactor pre-emptively would have been YAGNI without a probe-quality demand. **This is a known structural gap**: a future change that wires a real platform value through any of those suppliers could legitimately re-fire them on the spoofed snapshot. Closing this gap is straightforward — apply the phase-3/4 nullable-supplier + ctx-fallback pattern uniformly across all 10 bucket-(c) probes — but is out of scope for Power-8's "close everything that fires" mandate.

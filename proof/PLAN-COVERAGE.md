...
- **B1-validator hardening follow-ups:** (1) ✅ DONE — narrowed device-cgroup `c*:*/b*:*` → 5 explicit char majors (mem/tty/misc/pts/binder), block dropped; live-proven boot+root non-priv, validator-PASS, commit `8ee0e21`. (2) ✅ AUDIT DONE — l0b seccomp promotion-readiness security audit (`proof/seccomp-promotion-audit/`, commit `6220053`) found: only +{arch_prctl,personality,setns} re-permitted; a hidden `mount` unconditional-ALLOW that unfilters MS_BIND; and a FALSE "no CAP_SYS_ADMIN" comment (CAP_SYS_ADMIN IS curated) — comment corrected `77daf6b`. Recommendation: PROMOTE-AFTER-ARG-FILTERS (setns→CLONE_NEWNS, personality allowlist). ✅ **ARG-FILTERS APPLIED + VALIDATED** (board-approved, commit `bded617`): personality→{persona==0, ==query} (blocks ASLR-off/W^X), setns→nstype==CLONE_NEWNS only (blocks PID/NET/USER-ns + nstype==0 escape). Empirically boot+root re-tested; adversarial-validator PASS with a raw-syscall negative test proving the dangerous forms are denied EPERM(1) (not theater) and magiskd genuinely uses CLONE_NEWNS (10/10). Evidence: `proof/seccomp-promotion-audit/APPLIED.md`. ✅ **PROMOTED + ENFORCED** (board-approved): profile flipped PROPOSAL→PINNED PRODUCTION (sha `d317a7a3…`, commit `8b1d740`, validator boot-re-tested), and the sha256 pin is now CODE-ENFORCED before boot via `verify_hardened_seccomp_pin()` — drift aborts before `docker run` (commit `8957ad5`, validator-proven `docker_reached=False` on drift). The full seccomp lifecycle (audit→comment-fix→arg-filters→promotion→pin→enforcement) is complete, every step validator-gated.
- **Pin-enforcement generalization:** ✅ DONE (commit `469723a`) — implemented `local-module-file-sha256-match` in the stability preflight (sha256 every module file vs pin → exit 78 on drift); also found+resynced 2 STALE pins (cpuinfo.spoofed + service.sh, stale since the legitimate Tensor-G2 fix `676d6c1`). Now all enforceable verification checks are code-enforced (digest-match + seccomp-l0b-sha256-match + local-module-file-sha256-match); the remaining `module-commit-match` targets `pending-runtime-verification` placeholder pins (magisk v27.2/rezygisk/lsposed — external artifacts not held, supply-chain-gated).
...
### Detection-cli scoring-panel parity (2026-06-01)
- The CLI snapshot-scoring panel was a 69-probe subset of the canonical 84; brought to **83 = canonical 84 MINUS the one genuinely live-only `KeystoreAttestationProbe`** (excluded as absence-noise — it flagged clean Pixels critical). Now snapshot scoring matches the canonical detector for everything snapshot-assessable. B2's honest score rose 0.1279→**0.1697 SUSPICIOUS** (2 genuine criticals). Commit `542069a`, validator-gated (clean fixtures 0-critical). Tracked capture-gap follow-ups for non-critical probes documented in `proof/detection-cli-panel-parity/RESULT.md`.
